function [u errplot energyplot iter] = genSplitBregman_step1solver_pshrink(Phi1, Phi2, Phi3, A, g, row, col, p, alpha, beta, mu, lambda1, lambda2, tol, N )
...
if nargin < num_params - 5
    beta = 1;
end
if nargin < num_params - 6
    alpha = 1;
end
...
while norm( u(:,2)-u(:,1) ) / norm(u(:,1)) > tol
    for i = 1:N
        ...
        d  = pshrink( Phi1(u(:,2))+b, alpha/lambda1, p ); 
        dx = pshrink( Phi2(u(:,2))+bx, beta/lambda2, p ); 
        dy = pshrink( Phi3(u(:,2))+by, beta/lambda2, p ); 
    end
